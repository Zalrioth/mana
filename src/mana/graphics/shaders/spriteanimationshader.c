...
int sprite_animation_shader_init(struct SpriteAnimationShader* sprite_animation_shader, struct GPUAPI* gpu_api, int depth_test) {
  ...
  if (vkCreateDescriptorSetLayout(gpu_api->vulkan_state->device, &layout_info, NULL, &sprite_animation_shader->shader.descriptor_set_layout) != VK_SUCCESS)
    return 0;
  ...
  if (vkCreateDescriptorPool(gpu_api->vulkan_state->device, &pool_info, NULL, &sprite_animation_shader->shader.descriptor_pool) != VK_SUCCESS) {
  ...
  shader_init(&sprite_animation_shader->shader, gpu_api->vulkan_state, "./assets/shaders/spirv/spriteanimation.vert.spv", "./assets/shaders/spirv/spriteanimation.frag.spv", NULL, vertex_input_info, gpu_api->vulkan_state->gbuffer->render_pass, color_blending, VK_FRONT_FACE_COUNTER_CLOCKWISE, depth_test, gpu_api->vulkan_state->msaa_samples, 0, VK_CULL_MODE_BACK_BIT);

  return 1;
}

void sprite_animation_shader_delete(struct SpriteAnimationShader* sprite_animation_shader, struct GPUAPI* gpu_api) {
  shader_delete(&sprite_animation_shader->shader, gpu_api->vulkan_state);

  vkDestroyDescriptorPool(gpu_api->vulkan_state->device, sprite_animation_shader->shader.descriptor_pool, NULL);
}
