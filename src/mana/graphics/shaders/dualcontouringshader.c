...
int dual_contouring_shader_init(struct DualContouringShader* dual_countouring_shader, struct GPUAPI* gpu_api) {
  ...
  if (vkCreateDescriptorSetLayout(gpu_api->vulkan_state->device, &layout_info, NULL, &dual_countouring_shader->shader.descriptor_set_layout) != VK_SUCCESS)
    return 0;
  ...
  if (vkCreateDescriptorPool(gpu_api->vulkan_state->device, &poolInfo, NULL, &dual_countouring_shader->shader.descriptor_pool) != VK_SUCCESS) {
  ...
  VkPipelineColorBlendAttachmentState color_blend_attachments[DUAL_CONTOURING_COLOR_ATTACHEMENTS];
  memset(color_blend_attachments, 0, sizeof(VkPipelineColorBlendAttachmentState) * DUAL_CONTOURING_COLOR_ATTACHEMENTS);
  for (int pipeline_attachment_num = 0; pipeline_attachment_num < DUAL_CONTOURING_COLOR_ATTACHEMENTS; pipeline_attachment_num++) {
  ...
  color_blending.attachmentCount = DUAL_CONTOURING_COLOR_ATTACHEMENTS;
